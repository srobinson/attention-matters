...
fn test_pairwise_drift_moves_closer() {
    ...
    let (activation, _) = QueryEngine::activate(&mut sys, "alpha beta gamma delta epsilon zeta");
...
fn test_manifest_contains_activated_ids() {
    ...
    let total_activated = result.activation.subconscious.len() + result.activation.conscious.len();
    assert_eq!(
