/// Golden ratio: (1 + √5) / 2.
/// Not referenced directly, but documents the derivation of `GOLDEN_ANGLE`.
#[allow(dead_code)]
pub(crate) const PHI: f64 = 1.618_033_988_749_895;

/// Golden angle in radians: 2π / φ²
pub(crate) const GOLDEN_ANGLE: f64 = 2.399_963_229_728_653_3;

/// Neighborhood radius: π / φ (radians on S³)
pub(crate) const NEIGHBORHOOD_RADIUS: f64 = 1.941_613_546_047_687_8;
...
pub(crate) const SLERP_THRESHOLD: f64 = 0.9995;
...
pub(crate) const PAIRWISE_DRIFT_MAX_MOBILE: usize = 200;

/// GC: minimum activation count to survive eviction.
/// Occurrences at or below this are candidates for garbage collection.
/// Exported for `am-store` GC pass configuration.
pub const ACTIVATION_FLOOR: u32 = 0;

/// GC: per-project DB size soft limit before GC triggers (50MB).
/// Retained for future size-based GC triggers; currently superseded
/// by config-driven `AM_DB_SIZE_MB`.
#[allow(dead_code)]
pub(crate) const DB_SOFT_LIMIT_BYTES: u64 = 50 * 1024 * 1024;

/// GC: target ratio of soft limit after aggressive eviction (80%).
/// Exported for `am-store` size-based GC computation.
...
/// Exported for `am-store` retention policy defaults.
